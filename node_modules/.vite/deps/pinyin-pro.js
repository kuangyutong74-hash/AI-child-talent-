...
function addCustomConfigToDict(config, dict, handleType) {
	for (let word in config) {
		const pinyinList = config[word].split(" ");
		splitString(word).forEach((char, index) => {
			...
			if (handleType === "replace" || handleType === "add" && !base) dict.set(char, pinyin);
			else {
				const merged = base;
				if (!current) dict.set(char, merged);
				if (!merged.split(" ").includes(pinyin)) dict.set(char, `${merged} ${pinyin}`.trim());
			}
		});
	}
}
var getCustomMultipleDict = () => {
...
var getAllPinyin = (char, surname = "off") => {
	...
	if (customMultipleDict.get(char)) pinyin = customMultipleDict.get(char).split(" ");
	else if (surname !== "off") {
...
var getSplittedPolyphonicList = (list) => {
	return list.map((item) => {
		if (!item.isZh) return [item];
		if (!item.result.includes(" ")) return [{
			origin: item.origin,
			result: item.result,
			isZh: true,
...
var handleType = (list, options) => {
	if (options.type !== "all") {
		...
		for (const item of list) result.add(item.result);
...
function formatNumToSymbol(pinyin) {
	...
	if (lastChar >= 0 && lastChar <= 4) {
		for (let key in toneMap) if (pinyin.includes(key)) return pinyin.slice(0, pinyin.length - 1).replace(key, toneMap[key][lastChar]) + suffixR;
		return pinyin + suffixR;
	} else return pinyin + suffixR;
}
function formatSymbolToNum(pinyin) {
	if (pinyin.endsWith("r") && pinyin.length > 1 && !/^[eēéěè]r$/.test(pinyin)) {
		...
		if (toneNum !== "" && toneNum !== "0") return `${getPinyinWithoutTone(pinyinWithoutR)}${toneNum}r`;
	}
