...
export default function StoryInput({
  ...
  return (
    <div className={`story-input-wrapper ${voiceFirst ? 'story-input-voice-first' : ''}`}>
      {voiceError && (
        <div className="voice-error">{voiceError}</div>
      )}

      {voiceFirst && (
        <div className="voice-first-zone">
          <button
            type="button"
            className={`voice-first-btn ${listening ? 'voice-btn-active' : ''}`}
            onClick={handleVoiceInput}
            disabled={disabled}
            title={listening ? '正在聆听，点击停止' : '按住大话筒，说出你的想法'}
          >
            {listening ? (
              <span className="voice-first-listening" aria-label="正在录音">⏺</span>
            ) : (
              <PngIcon name="action-microphone" size={52} />
            )}
          </button>
          <p className="voice-first-label">
            {listening ? (interim || '正在聆听...请说话') : '点大话筒，说出你的想法'}
          </p>
        </div>
      )}
      ...
      <p className="story-input-hint">
        {voiceFirst ? '也可以打字哦 · 说完话会自动发送' : '按 Enter 发送，Shift+Enter 换行 · 支持语音输入'}
      </p>
    </div>
  );
}
