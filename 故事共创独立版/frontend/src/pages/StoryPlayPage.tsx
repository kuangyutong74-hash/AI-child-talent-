...
export default function StoryPlayPage() {
  ...
  const { notify, isGuideActive } = useGuide();
  const { ageGroup } = useChannel();
  const isYoung = ageGroup === '4-7';
  const { speak, stop } = useTTS();
  const chatEndRef = useRef<HTMLDivElement>(null);
  const loadingRef = useRef(false);
  const storyStartedRef = useRef(false);
  const aiOpeningDoneRef = useRef(false);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      aiOpeningDoneRef.current = false;
  ...
  // 新手引导：故事导演的开场白讲完后，自动推进到输入框步骤
  useEffect(() => {
    if (aiOpeningDoneRef.current) return;
    if (state.messages.length > 0 && !state.isStreaming) {
      aiOpeningDoneRef.current = true;
      notify('first-turn-finished');
    }
  }, [state.messages.length, state.isStreaming, notify]);

  // 幼儿通道：AI 讲完一段后自动朗读（识字有限，听故事为主）。
  // 触发时机：一段流式输出刚结束；或引导遮罩刚关闭（首次使用时开场白被引导压住，关闭后补读）。
  // 历史消息恢复加载时不触发（wasStreaming 从未变过 true）。
  const wasGuideActiveRef = useRef(false);
  useEffect(() => {
    const wasStreaming = wasStreamingRef.current;
    const guideJustClosed = wasGuideActiveRef.current && !isGuideActive;
    wasStreamingRef.current = state.isStreaming;
    wasGuideActiveRef.current = isGuideActive;
    if (!isYoung) return;
    const shouldRead = (wasStreaming && !state.isStreaming) || guideJustClosed;
    if (!shouldRead || state.isStreaming) return;
    const lastAi = [...state.messages].reverse().find((m) => m.role === 'ai');
    if (!lastAi || lastAi.isStreaming) return;
    if (spokenMessageIdsRef.current.has(lastAi.id)) return;
    spokenMessageIdsRef.current.add(lastAi.id);
    speak(lastAi.content);
  }, [state.messages, state.isStreaming, isYoung, isGuideActive, speak]);
  ...
  async function handleSend(text: string) {
    ...
    try {
      ...
      // 新手引导：完成一轮发言后，故事场景引导结束
      notify('first-turn-sent');
    } catch {
      // Errors handled in useSSE
    }
  }
  ...
  // 幼儿通道默认大字，方便识读
  const [fontSize, setFontSize] = useState<'s' | 'm' | 'l'>(isYoung ? 'l' : 'm');
  ...
  return (
    ...
    <div className="story-play-page">
      ...
      {/* Chat area */}
      <div className="story-chat-area" data-guide="story-chat-area">
        {showStartHint && (
      ...
      {/* Input area */}
      <div className="story-play-bottom" data-guide="story-play-bottom">
        {state.isEnding ? (
