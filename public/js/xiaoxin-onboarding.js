...
(function () {
  ...
  var PAGE_TOURS = {
    ...
    '/home.html': [
      ...
      },
      {
        target: '.top-actions',
        title: '随时找到我 👋',
        content: '点击<b>"我的"</b>可以查看你的主页，<b>拼音</b>开关让文字更好读～<br>想再看一遍引导的话，点右上角<b>"引导"</b>按钮就行啦！',
        position: 'bottom',
        spotlightPadding: 10,
      },
    ],
    ...
    '/profile.html': [
      {
        target: null,
        title: '你的个人主页 👤',
        content: '在这里可以查看和编辑你的信息，<br>让小新更了解你～',
        position: 'center',
        spotlightPadding: 0,
      },
    ],
  ...
  function init(force) {
    ...
    function finish() {
      ...
      // 仅首次自动弹出时通知服务端；手动重看不改动 onboardingDone
      if (!isForceRun) {
        window.__onboardingDone = true;
        if (window.__dbg) window.__dbg('dbg-fetch', '发送 POST /api/auth/onboarding-complete ...', '#ff0');

        fetch('/api/auth/onboarding-complete', { method: 'POST', credentials: 'same-origin' })
          .then(function (res) {
            if (window.__dbg) window.__dbg('dbg-fetch', 'status=' + res.status + (res.ok ? ' OK' : ' FAIL'), res.ok ? '#0f0' : '#f44');
            console.log('[onboarding-complete] 响应状态:', res.status);
            return res.json();
          })
          .then(function (data) {
            if (window.__dbg) window.__dbg('dbg-fetch', JSON.stringify(data) + (data.ok ? ' ✅' : ' ❌'), data.ok ? '#0f0' : '#f44');
            console.log('[onboarding-complete] 响应内容:', data);
          })
          .catch(function (err) {
            if (window.__dbg) window.__dbg('dbg-fetch', '❌ ' + (err.message || err), '#f44');
            console.error('[onboarding-complete] 请求失败:', err);
          });
      } else {
        if (window.__dbg) window.__dbg('dbg-fetch', 'isForceRun=true, 跳过请求', '#888');
      }
  ...
  // 自动弹出不再是脚本加载时无条件判断，而是由 home.html 在拿到 /api/auth/me 的
  // onboardingDone 字段后，在 window.__onboardingDone 上设置标记，
