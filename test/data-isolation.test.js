...
describe('数据隔离', function () {
  ...
  it('正式 data 目录的 users.json 未被修改', function () {
    var after = fileSnapshot(path.join(realDataDir, 'users.json'));
    var before = beforeSnapshots['users.json'];
    assert.deepStrictEqual(after, before,
      'users.json 的元数据应与测试前完全一致');
  });

  it('正式 data 目录的 sessions.json 未被修改', function () {
    var after = fileSnapshot(path.join(realDataDir, 'sessions.json'));
    var before = beforeSnapshots['sessions.json'];
    assert.deepStrictEqual(after, before,
      'sessions.json 的元数据应与测试前完全一致');
  });

  it('正式 data 目录的 bindings.json 未被修改', function () {
    var after = fileSnapshot(path.join(realDataDir, 'bindings.json'));
    var before = beforeSnapshots['bindings.json'];
    assert.deepStrictEqual(after, before,
      'bindings.json 的元数据应与测试前完全一致');
  });
