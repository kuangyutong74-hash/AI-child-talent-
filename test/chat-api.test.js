...
describe('POST /chat/session (V1)', function () {
  ...
  it('学生输入应出现在 user 消息中', async function () {
    ...
    var fetchBody = captured[captured.length - 1].body;
    var userMessages = fetchBody.messages.filter(function (m) { return m.role === 'user'; });
    assert.ok(userMessages.length >= 1);
    assert.strictEqual(userMessages[userMessages.length - 1].content, message);
  });
...
describe('V2 集成', function () {
  ...
  it('V2 不执行 legacy choice rewrite', async function () {
    ...
    assert.strictEqual(res.status, 200);
    // 仍只有 2 次调用（analyze + generate），没有额外的 rewrite 调用
    // 注意：如果触发 repair，可能有 3 次。
    // 关键：不应有额外的 rewrite 调用（analyze + generate，最多 + 1 repair）。
    assert.ok(v2Captured.length <= 3);
  });
  ...
  it('repair 请求只包含 validation code', async function () {
    ...
    // 第三次调用是 repair
    if (v2Captured.length >= 3) {
      var repairCall = v2Captured[2];
      assert.ok(repairCall.body.messages);
      var userContent = repairCall.body.messages[0].content;
