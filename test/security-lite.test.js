 ...
 *
 * 覆盖：
 * - Cookie 合同（Secure / HttpOnly / SameSite / Path / Max-Age）
...
// ============================================================
//  A. Cookie 合同测试（纯函数，不需要服务器）
// ============================================================

describe('A. Cookie 合同', function () {
  var cookieHelper = require('../lib/infra/cookie-helper');
  var buildCookie = cookieHelper.buildSessionCookie;

  it('1. development Cookie 不含 Secure', function () {
    var c = buildCookie('test-token', false);
    assert.ok(c.indexOf('HttpOnly') >= 0);
    assert.ok(c.indexOf('SameSite=Lax') >= 0);
    assert.ok(c.indexOf('Path=/') >= 0);
    assert.ok(c.indexOf('Secure') < 0, 'must NOT contain Secure');
  });

  it('2. production Cookie 含 Secure', function () {
    var c = buildCookie('test-token', true);
    assert.ok(c.indexOf('HttpOnly') >= 0);
    assert.ok(c.indexOf('SameSite=Lax') >= 0);
    assert.ok(c.indexOf('Secure') >= 0, 'must contain Secure');
  });

  it('3. COOKIE_SECURE=true 含 Secure', function () {
    var c = buildCookie('abc-123', true);
    assert.ok(c.indexOf('Secure') >= 0);
  });

  it('4. COOKIE_SECURE=false 不含 Secure', function () {
    var c = buildCookie('abc-123', false);
    assert.ok(c.indexOf('Secure') < 0);
  });

  it('5. Cookie 含 HttpOnly', function () {
    var c = buildCookie('t', false);
    assert.ok(c.indexOf('HttpOnly') >= 0);
  });

  it('6. Cookie 含 SameSite=Lax', function () {
    var c = buildCookie('t', false);
    assert.ok(c.indexOf('SameSite=Lax') >= 0);
  });

  it('7. Cookie 含 Path=/', function () {
    var c = buildCookie('t', false);
    assert.ok(c.indexOf('Path=/') >= 0);
  });

  it('8. 登出 Cookie Max-Age=0', function () {
    var c = buildCookie('', true);
    assert.ok(c.indexOf('Max-Age=0') >= 0);
    assert.ok(c.indexOf('token=') >= 0);
  });

  it('9. 登录和登出 Cookie 安全属性一致', function () {
    var login = buildCookie('tok', true);
    var logout = buildCookie('', true);
    ...
    var la = extractAttrs(login);
    var loa = extractAttrs(logout);

    // Max-Age 不同但其他属性应一致
    assert.strictEqual(loa.HttpOnly, true);
    assert.strictEqual(loa['SameSite'], 'Lax');
    assert.strictEqual(loa.Path, '/');
    assert.strictEqual(loa.Secure, true);
    assert.strictEqual(la.Secure, loa.Secure);
    assert.strictEqual(la.Path, loa.Path);
    assert.strictEqual(la['SameSite'], loa['SameSite']);
  });

  it('10. 空 token 安全处理为登出', function () {
    var c = buildCookie('', false);
    assert.ok(c.indexOf('token=') >= 0);
    assert.ok(c.indexOf('token=;') >= 0 || c.indexOf('token=; ') >= 0 || c.indexOf('Max-Age=0') >= 0);
  });
});
...
describe('I. 模块导出', function () {
  ...
  it('45. cookie-helper 导出 buildSessionCookie', function () {
    var mod = require('../lib/infra/cookie-helper');
    assert.strictEqual(typeof mod.buildSessionCookie, 'function');
    assert.strictEqual(typeof mod.TOKEN_MAX_AGE_SECONDS, 'number');
  });
  ...
  it('47. 模块不读取 .env', function () {
    // 验证：三个新模块都不包含 process.env（除了 env-config.js 本身）
    var cookiePath = path.join(__dirname, '..', 'lib', 'infra', 'cookie-helper.js');
    var secPath = path.join(__dirname, '..', 'lib', 'infra', 'security-headers.js');

    var cookieCode = fs.readFileSync(cookiePath, 'utf-8');
    var secCode = fs.readFileSync(secPath, 'utf-8');

    assert.ok(cookieCode.indexOf('process.env') < 0,
      'cookie-helper must not read process.env');
